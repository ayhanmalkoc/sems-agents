...
import { adoptStudioOutput, exportStudioOutput, listStudioOutputsForSessions, type AdoptStudioOutputInput, type StudioExportFormat } from '@craft-agent/shared/studio'
...
export function registerStudioHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  server.handle(RPC_CHANNELS.studio.EXPORT, async (_ctx, workspaceId: string, outputId: string, format: StudioExportFormat) => {
    if (!['html', 'zip'].includes(format)) throw new Error('Export format must be html or zip')
    const root = workspaceRoot(workspaceId)
    ...
    const output = exportStudioOutput(sessionPath(root, existing.metadata.sessionId), outputId, format)
