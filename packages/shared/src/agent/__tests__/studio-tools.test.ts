...
describe('studio tool', () => {
  it('formats status, templates, list, show, create, project, update, quality, export, and adopt', async () => {
    ...
    expect((await executeStudioCommand('scenarios', mock)).content[0].text).toContain('agent-platform')
    expect((await executeStudioCommand('scenario agent-platform', mock)).content[0].text).toContain('Agent AI Platform')
    expect((await executeStudioCommand('recommend {"prompt":"animated agent AI platform"}', mock)).content[0].text).toContain('web-prototype')
    ...
    expect((await executeStudioCommand('assets created', mock)).content[0].text).toContain('none')
  ...
  it('rejects malformed commands', async () => {
    ...
    expect((await executeStudioCommand('scenario', mock)).content[0].text).toContain('scenario requires a scenario id')
    expect((await executeStudioCommand('recommend {}', mock)).content[0].text).toContain('recommend requires a prompt')
