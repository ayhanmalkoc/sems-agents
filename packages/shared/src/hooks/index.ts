...
function matcherMatches(matcher: HookMatcher, input: HookInput): boolean {
  ...
  if (matcher.metadata && Object.entries(matcher.metadata).some(([key, value]) => input.metadata?.[key] !== value)) return false
...
export class HookEngine {
  ...
  private decideBuiltin(hook: BuiltinHookDefinition, input: HookInput): HookOutput { const policy = this.policy(); if (hook.id === 'secret_scan_prompt') return policy.secretGuard === 'off' ? { decision: 'allow', reason: 'Secret guard is off.' } : containsSecret(input.prompt ?? input.tool_input) ? { decision: 'block', reason: 'Hook blocked prompt: sensitive credentials or secrets detected.' } : { decision: 'allow', reason: 'No prompt secrets detected.' }; if (hook.id === 'secret_scan_tool_input') return policy.secretGuard === 'off' ? { decision: 'allow', reason: 'Secret guard is off.' } : containsSecret(input.tool_input) ? { decision: 'block', reason: 'Hook blocked tool input: sensitive credentials or secrets detected.' } : { decision: 'allow', reason: 'No tool input secrets detected.' }; if (hook.id === 'tool_prerequisite_guard') return { decision: 'observe', reason: policy.prerequisiteGuard === 'enforce' ? 'Prerequisite guard is enforced by the existing prerequisite manager.' : 'Prerequisite guard observe-only.' }; if (hook.id === 'workspace_boundary_guard') { if (!looksWorkspaceRisky(input)) return { decision: 'allow', reason: 'No workspace boundary risk detected.' }; if (policy.workspaceBoundary === 'block') return { decision: 'block', reason: 'Hook blocked risky workspace boundary operation.' }; if (policy.workspaceBoundary === 'ask') return { decision: 'ask', reason: 'Hook requires approval for risky workspace boundary operation.' }; return { decision: 'observe', reason: 'Observed risky workspace boundary operation.' } } if (hook.id === 'memory_learn_on_stop') return input.metadata?.reason === 'session_complete' ? { decision: 'modify', reason: 'Memory Brain task will learn from the completed session.' } : { decision: 'allow', reason: 'Stop is not a session completion.' }; if (hook.id === 'memory_explicit_remember_on_prompt') { const prompt = String(input.prompt ?? '').toLowerCase(); return /\b(remember this|remember that|remember it|bunu hatirla|bunu hatırla|sunu hatirla|şunu hatırla|bunu hafizaya al|bunu hafızaya al|not et)\b/i.test(prompt) ? { decision: 'modify', reason: 'Explicit remember request detected for Memory Brain.' } : { decision: 'allow', reason: 'No explicit remember request detected.' } }; if (hook.id === 'tool_audit_log') { if (policy.toolAudit === 'off') return { decision: 'allow', reason: 'Tool audit is off.' }; if (containsSecret(input.tool_response)) return { decision: 'redact', reason: `Audited and redacted tool ${input.tool_name ?? 'unknown'}.`, redacted_response: typeof input.tool_response === 'string' ? redactSecrets(input.tool_response) : JSON.parse(redactSecrets(JSON.stringify(input.tool_response))) }; return { decision: 'observe', reason: `Audited tool ${input.tool_name ?? 'unknown'}.` } } if (hook.id === 'validation_summary_on_stop') return { decision: 'observe', reason: 'Captured stop validation summary context.' }; return { decision: 'observe', reason: 'Hook observed event.' } }
