...
import { createMemory, updateMemory, deleteMemory, loadMemories, loadMemorySuggestions, searchMemories, createMemorySuggestion, approveMemorySuggestion, rejectMemorySuggestion } from '@craft-agent/shared/memory'
        ...
        mergeSessionScopedToolCallbacks(sid, {
          ...
          memoryFns: {
            ...
            suggestFromSession: async (sessionId) => {
              ...
              const suggestion = createMemorySuggestion(managed.workspace.rootPath, {
                type: 'workflow_learning',
                scope: 'session',
                title: `Review session ${sessionId}`,
                content: `Candidate memory from session "${target.name ?? sessionId}". Review the session before approval.`,
                reason: 'Generated by suggest-from-session; requires human review before use.',
