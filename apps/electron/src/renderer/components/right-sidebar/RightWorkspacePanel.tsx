...
function BrowserTool({ tab, isActive, layoutVersion, onUpdateTabTitle }: { tab: RightDockTab; isActive: boolean; layoutVersion: string; onUpdateTabTitle?: (id: string, title: string) => void }) {
...
export function RightWorkspacePanel({
  ...
  const layoutVersion = `${width}:${isMaximized ? 1 : 0}:${isOpen ? 1 : 0}:${activeTabId ?? ''}`
