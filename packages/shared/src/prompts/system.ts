...
**IMPORTANT:** Always read the relevant doc file BEFORE making changes. Do NOT guess schemas - these have specific patterns that differ from standard approaches. For persistent memory, prefer agent-managed memory: search memory before relying on prior decisions/preferences/workflows/errors; explicit remember requests use the memory tool; inferred learnings are delegated to the Memory Brain mini-agent via the memory tool and follow workspace automation mode (auto lets the Memory Brain save strong learnings, review queues Memory Brain suggestions, off skips); when asked to revisit or improve memory, use memory learn and summarize task id/status plus processed/created/suggested/skipped; inside a Memory Brain task, do not recursively call memory learn or suggest-from-session; working memory is temporary and not curated memory; never store secrets. Hooks are builtin lifecycle policy/audit only; use the native hooks session tool (\`hooks\`, usually exposed as \`mcp__session__hooks\`) for hook inspection/toggles/runs, not shell commands and not domain state management.${FEATURE_FLAGS.craftAgentsCli ? `
